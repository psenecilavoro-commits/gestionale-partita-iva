...
def mostra_veicoli_reali(client, anno):
    ...
    with st.expander("Registra un veicolo e la percorrenza effettiva"):
        ...
        vid = st.selectbox("Veicolo per la percorrenza", list(ids), format_func=lambda k: ids[k]["label"], key=f"km_veicolo_{anno['id']}")
        mese = st.selectbox("Mese percorrenza", list(range(1, 13)), format_func=lambda m: MESI[m-1], key=f"km_mese_{anno['id']}")
        try:
...
def mostra_costi_reali(client, anno):
    ...
    try:
        ...
        return
    st.dataframe([{"Data": r["expense_date"], "Descrizione": r["description"], "Importo": r["gross_amount"],
                   "IVA compresa": r["amount_includes_vat"], "Origine": r.get("notes")} for r in costi], hide_index=True)
    from quadro_mensile import _csv_bytes
    st.download_button("Esporta spese registrate CSV", _csv_bytes(costi), f"spese_{anno['fiscal_year']}.csv")
    if anno["status"] != "open":
        return
    # Le categorie auto continuano ad usare i moduli che assegnano il veicolo.
    ...
    if mancanti:
        with st.expander("Prepara una categoria senza inserire stime o spese"):
            codice = st.selectbox("Categoria da preparare", mancanti, format_func=lambda k: CONFIG[k][1], key=f"categoria_reale_{anno['id']}")
    ...
    scelta = st.selectbox("Spesa effettiva da registrare o modificare", [None, *rows],
                         format_func=lambda k: "Nuova spesa" if k is None else f"{rows[k]['expense_date']} · {rows[k]['description']}", key=f"spesa_reale_{anno['id']}")
    r = rows.get(scelta)
    with st.form(f"spesa_form_{anno['id']}_{scelta}"):
        cid = st.selectbox("Categoria spesa", list(cats), index=list(cats).index(r["category_id"]) if r else 0, format_func=lambda k: cats[k]["name"])
    ...
    if invia:
        try:
            ...
            if elimina:
                ...
                dati = {"category_id": cid, "expense_date": giorno.isoformat(), "description": descrizione.strip(),
                        "gross_amount": str(importo_valido(importo)), "amount_includes_vat": include,
                        "fiscal_competence_year": int(anno["fiscal_year"]),
                        "notes": (r.get("notes") if r and str(r.get("notes") or "").startswith("DATI DI PROVA") else "Spesa effettiva dichiarata; percentuali da verificare")}
                # Conserva percentuali storiche quando la categoria non cambia.
